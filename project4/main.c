...
int dictionary_insert_from_raw(struct dictionary* dicts, char * raw, struct encode_output* output_array, unsigned long total_words )
{
	...
	for(int i=0;i<dicts->current_largest;i++)
	{
		if(dicts->array_data[i] == hashvalue)
		{
			...
			strcpy(dicts->array[i][dicts->collision_data[i]].string, raw);
			dicts->array[i][dicts->collision_data[i]].location_rank = dicts->collision_data[i];
			dicts->array[i][dicts->collision_data[i]].location = hashvalue;
			dicts->array[i][dicts->collision_data[i]].index = i;
	...
	strcpy(dicts->array[dicts->current_largest-1][0].string, raw);
...
int main(int argc, char** argv){
	...
	if(strcmp("-e",argv[1])==0 && argc== 4)
	{
		...
		free(word);
		free(dicts-> array_data);
		free(dicts-> collision_data);
	...
	}else if(argc == 4 && strcmp(argv[1],"-q")==0)
	{
		...
		while(fgets(line,99, encode_ptr)!=NULL)
		{
			...
			{
				printf("Find the string %s and it appears%s times\n", raw, count_string);
				break;
			}
